    ...
    let audio_thread = thread::spawn(move || {
        loop {
            ...
            while *restart_playlist==false{
                ...
                let (result,wait_time_out_result) = restart_condvar_clone.wait_timeout(restart_playlist, Duration::from_secs(20)).unwrap();
            ...
            if timeout_expired {
                ...
                println!("No action taken within 20 seconds. Please rerun your program by providing the file path for where your songs are.");
            ...
            if current_song < playlist.len() {
               ...
               println!("restart playlist {}",restart_playlist );
                if !*restart_playlist {
                    println!("Your playlist queue has completed. Press 'r' to repeat, 'q' to quit.");
                    break;
                } else {
    ...
    let user_input_thread = thread::spawn(move || {
        ...
        for line in stdin.lock().lines() {
            ...
            match line.as_str() {
                ...
                "r" =>{
                    let mut restart_playlist = restart_playlist.lock().unwrap();
                   if !*restart_playlist {
                   *restart_playlist=true;
                   println!("repeat true");
                       ...
                       *restart_playlist=false;
                       println!("repeat false");
                   }
                   restart_condvar.notify_one();
                }
                "f" => {
                ...
                _ => println!("invalid command {} .type 'help' to see available commands'",line.as_str()),
            }
        }
    });
